...
show_manual_menu() {
    while true; do
        clear
        show_main_banner
        dashboard_header "MANUAL TROUBLESHOOTING MODE (Offline)" "$MR_SKY"
        dashboard_row "Hostname:" "$(hostname)"
        dashboard_row "Health:" "$(calc_health_score)/100" "ok"
        dashboard_footer
        echo ""
...
show_solution() {
    ...
    for step in "$@"; do
        if [[ "$step" == "   "* ]]; then
            # This is a command
            local clean_cmd=$(echo "$step" | sed 's/^ *//')
            ((cmd_count++))
            cmd_list+=("$clean_cmd")
            echo -e "      ${MR_GOLD}[$cmd_count] $clean_cmd${MR_NC}"
        else
    ...
    if [[ "$cmd_count" -gt 0 ]]; then
        if confirm_action "Would you like to copy a command to clipboard?" "n"; then
            if [[ "$cmd_count" -eq 1 ]]; then
                local to_copy="${cmd_list[0]}"
                echo "$to_copy" | xclip -selection clipboard 2>/dev/null && msg_ok "Copied: $to_copy" || msg_info "Command: $to_copy (Copy manually if xclip missing)"
            else
                styled_prompt "Enter command number to copy (1-$cmd_count)"
                if [[ "$REPLY" =~ ^[0-9]+$ ]] && [[ "$REPLY" -le "$cmd_count" ]] && [[ "$REPLY" -gt 0 ]]; then
                    local index=$((REPLY - 1))
                    local to_copy="${cmd_list[$index]}"
                    echo "$to_copy" | xclip -selection clipboard 2>/dev/null && msg_ok "Copied!" || msg_info "Command: $to_copy"
                else
