...
pulse_text() {
    ...
    local color="${2:-$MR_CYAN}"
    for i in {1..3}; do
        echo -ne "\r  ${color}${MR_BOLD}⚡ $text${MR_NC}   "
        sleep 0.2
        echo -ne "\r  ${color}${MR_BOLD}   $text${MR_NC}   "
        sleep 0.2
    done
...
loading_screen() {
    ...
    echo ""
    echo ""
    center_text "${MR_CYAN}${MR_BOLD}Initializing ${msg}...${MR_NC}"
    echo ""
    ...
    for ((i=1; i<=width; i++)); do
        ...
        local bar=""
        for ((j=1; j<=i; j++)); do bar+="━"; done
        for ((j=i+1; j<=width; j++)); do bar+=" "; done
        
        printf "\r  "
        center_text "${MR_DIM}┠${MR_CYAN}${bar}${MR_DIM}┨ ${MR_WHITE}${pct}%%${MR_NC}"
...
spinner_start() {
    ...
    (
        local i=0
        IFS=')' # For bounce type split
        local chars_arr
        if [[ "$type" == "bounce" ]]; then
            read -ra chars_arr <<< "$spin_chars"
        else
            chars_arr=()
            for ((j=0; j<${#spin_chars}; j++)); do chars_arr+=("${spin_chars:j:1}"); done
        fi
        
        while true; do
            local char="${chars_arr[$i]}"
            printf "\r  ${MR_CYAN}[${char}]${MR_NC} ${MR_DIM}%s${MR_NC}" "$msg"
            i=$(( (i + 1) % ${#chars_arr[@]} ))
            sleep 0.1
        done
    ) &
...
spinner_stop() {
    ...
    # Ensure SPINNER_PID is defined before checking
    ...
    # Carriage return + clear existing line to prevent artifacts
    printf "\r\033[K"
    case "$status" in
        ok)   msg_ok "$msg" ;;
        fail) msg_fail "$msg" ;;
...
show_large_ascii_banner() {
    ...
    echo '    ║                                                                                                                              ║'
    echo '    ║                                                                                                                              ║'
    echo -e "    ║    ${MR_WHITE}${MR_BOLD}MR-LINMACHANIC - The Machine That Repairs Linux${MR_NC}${MR_CYAN}                                                                 ║"
    echo -e "    ║    ${MR_DIM}Version ${MR_VERSION} (${MR_CODENAME})${MR_NC}${MR_CYAN}                                                                                                    ║"
    echo -e "    ║    ${MR_DIM}Author: Madan Raj${MR_NC}${MR_CYAN}                                                                                                            ║"
    echo -e "    ║    ${MR_DIM}LinkedIn: www.linkedin.com/in/madanraj0${MR_NC}${MR_CYAN}                                                                                     ║"
    echo '    ║                                                                                                                              ║'
    echo '    ║                                                                                                                              ║'
    echo '    ╚════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╝'
    echo -e "${MR_NC}"
...
# ── Main banner ──
show_main_banner() {
    show_large_ascii_banner "Main Menu" "Linux System Analysis & Repair Tool"
}
# ── Dashboard components ──
dashboard_header() {
    local title="$1"
    local color="${2:-$MR_CYAN}"
    # Target total width: 50 characters inside the box
    # Format: "╔══ TITLE ════════════╗"
    local total_width=50
    local prefix_len=3  # "╔══ "
    local suffix_len=2  # " ╗"
    local available_width=$((total_width - prefix_len - suffix_len))
    
    if [[ ${#title} -gt $available_width ]]; then
        # Truncate title if too long
        title="${title:0:$((available_width-3))}..."
    fi
    
    local pad_count=$((available_width - ${#title}))
    if [[ $pad_count -lt 0 ]]; then pad_count=0; fi
    
    echo -e "  ${color}╔══ ${MR_NC}${MR_WHITE}${title}${MR_NC}${color}$(printf '═%.0s' $(seq 1 $pad_count))╗${MR_NC}"
}

dashboard_footer() {
    local color="${1:-$MR_CYAN}"
    echo -e "  ${color}╚$(printf '═%.0s' $(seq 1 50))╝${MR_NC}"
}
...
dashboard_row() {
    ...
    # Format: "   Key:           Value    ●"
    printf "   ${MR_DIM}%-15s:${MR_NC} ${MR_WHITE}%-25s${MR_NC} %b\n" "$key" "$value" "$status_icon"
}
...
dashboard_bar() {
    ...
    local color
    if [[ $value -ge 80 ]]; then color="$MR_GREEN"
    elif [[ $value -ge 50 ]]; then color="$MR_YELLOW"
    elif [[ $value -ge 25 ]]; then color="$MR_ORANGE"
    else color="$MR_RED"; fi
    ...
    # Format: "  Label:           Info"
    #         "    Label:           █████████░░░░░░░░░░░░ 85%"
    printf "  ${MR_DIM}%-15s:${MR_NC} ${MR_WHITE}%-25s${MR_NC}\n" "$label" "$info"
    printf "    ${MR_DIM}%-15s:${MR_NC} ${color}%s${MR_NC} ${MR_WHITE}%3d%%${MR_NC}\n" "$label" "$bar" "$value"
}
